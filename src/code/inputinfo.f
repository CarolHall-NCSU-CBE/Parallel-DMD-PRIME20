! ============================================================================
! Subroutine inputinfo accesses the positions and velocities of the beads 
! as well as the arrays identity, aa, bdln, hp, chnnum, sigma, welldia, epsilon  	  
...
!LR: Added a third species numbeads variable. I did not have a numbeadstotal, due to those variables only being used rarely in the code
	...
	integer tempfside1(chnln1), tempfside2(chnln2), fs
...
!LR: Changed a hardcoded 2-species variable reference to a noptotal variable
...
#endif
      	read(parafs1,*) tempfside1
	do fs = 1, chnln1
		fside1(fs) = tempfside1(fs)
	enddo
      	l=1
	...
	if (chaptype .eq. 1) then
      		read(parafs2,*) tempfside2
		do fs = 1, chnln2
			fside2(fs) = tempfside2(fs)
		enddo
      		l=1
      		do k=1,chnln2 
...
89	format(i6,2x,i5,2f8.5)
      	...
      	write(6,*) hp(1:(numbeads1+numbeads2))
      	write(6,*)   
      	write(6,*)'***********************************************'
